...
清单是**发给葡萄牙客户**的，所以 Excel 和 PDF 内容**全部是欧洲葡萄牙语**
（app 界面仍然是英文——那是给店里用的）：
Foto / Produto / EAN / Categoria / **PVP** / Preco grossista / Disponibilidade。

| 价格 | 规则 |
|---|---|
| **PVP** | app 零售价 **+ €2**（`PVP_UPLIFT`）。给客户看的建议零售价高于收银台价。 |
| **Preço grossista** | app 批发价 **− 导出时填的折扣**（`discount`），不会低于 0。 |

**折扣只给「一瓶香水该有的容量」**（`discount_applies()`）：
香水分类 **且 60ml ≤ 容量 ≤ 120ml** **且 不是身体喷雾**。

**两端都要卡**，这不是「大瓶」而是「正常一瓶香水」：
50ml 本身就便宜，同样减几欧会把利润减掉大半；
150–250ml 是身体喷雾，按毫升卖的，不是按香味卖的。
容量这一关已经挡掉 200ml 的喷雾，浓度那一关是为了防 100ml 装的身体喷雾漏过去。

**没记录容量的产品不打折**。这是有意的：多打了折是悄悄少赚，
少打了折是看得见、也容易补。目前 238 个香水里**有 113 个没有容量**，
所以它们不会参与折扣——要让它们参与，把 `volume_ml` 填上即可。

**PVP 永远不打折**。建议零售价不是批发价，
批发让利不应该顺手把货架上的标价也改了。

折扣框在导出对话框里，**每次导出单独填**，不改动店里的任何数据——
同一批产品可以给不同客户报不同价格。支持 `2,50` 这种葡萄牙写法。

`PVP_UPLIFT` 是一个常量，改一次就全改，不用一个个产品去调。
...
这条规则**不写进清单**——客户不需要知道我们留了几件样品，只需要知道能不能买到。

**低库存 + 有货在途 = 两行**：产品那一行照常写 `Stock reduzido`（黄），
紧接着**下面加一行** `Brevemente em stock`（蓝）。
这两件事都是真的，客户两件都想知道：现在架上还有一点，而且还有货在路上。
Excel 和 PDF 都是这样。
