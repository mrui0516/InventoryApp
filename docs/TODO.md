...
**背景**：`db.sqlite3` + `media/`（79MB）**无任何备份**；且本地领先 `origin/master` **34 个提交**，GitHub 上没有近期工作 → 代码也没有异地副本。

- [ ] `git push` 把 34 个提交推到 GitHub（先确认没有密钥被误提交）
- [ ] 待定：备份目标位置（本地另一目录 / 外接盘 / 云同步文件夹）
- [ ] 待定：备份脚本（复制 `db.sqlite3` + 打包 `media/`，文件名带时间戳，保留最近 N 份）
- [ ] 待定：调度方式（Windows 任务计划程序 / 手动）
