...
- 2026-08-25: **修复供应商到货时效大量漏算。** 212 张已收货的入库单里**只有 16 张被统计**。根因有两个：(1) `InboundOrder.status` 的**默认值就是 `'received'`**，所以直接建的单 `received_at` 是 NULL —— 40 张这样的单永远算不了；(2) 服务层用 `received_at > created_at` **猜**"这行有没有真实数据"，把同天下单同天到货的单也悄悄丢掉。改为记录**事实**而不是猜：新增 `InboundOrder.placed_at`（下单时间），**两个时间戳都由 model 自己打**（`save()` 打 placed_at，新增 `mark_received()` 打 received_at），任何路径建单/收货都不会漏，包括 admin。统计规则变成「两个时间戳都在就算，缺一个就不算」。历史单没有 `placed_at` 的**永远不参与**（没有诚实的办法凭空造下单时间）；迁移 0049 只回填两种确定情况：**还没收货的暂定单**（这样它们落地时能算）和**原本已被统计的那 16 张**（否则现有平均值会全部消失——已验证回填前后 7 个供应商的平均值完全一致）。新增 `lead_time_report` 命令逐单打印 计入/跳过 + 跳过原因，用来查"为什么这单没算"。405 通过。
