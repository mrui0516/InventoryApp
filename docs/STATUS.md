...
- 2026-08-17：**底部标签栏视觉重做**——从"白底+灰图标"的素样式改成**与侧边栏同色系的深色导航条**(navy 渐变、顶部细描边+上投影);选中项用 **Material-3 风格高亮胶囊**(图标坐在蓝色圆角底上、文字转白加粗),点按有轻微缩放反馈。手机顶栏(深)+底栏(深)包住浅色内容,视觉统一。⚠ 改的是 app.css,PA 需 `collectstatic` 才生效。
