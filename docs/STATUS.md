...
- 2026-08-17: **Deleting a product no longer destroys anything + decants drop to 10.** (1) Delete used to cascade the product away, and a superuser's `force_delete` also removed its purchase/sales history and any order left empty — i.e. it deleted the books. It now only clears the product's photos: the product, its batches, its sales and their orders all stay, and the product keeps showing everywhere (owner's choice), so reporting and profit are untouched. The delete gating flags and the force-delete path are gone and the UI is relabelled 'Remove Photos' so it states what it does. Regression test asserts purchases/sales/orders survive and only images go. (2) Shopify decant availability `DECANT_AVAILABLE` 99 → **10** (5ml/10ml show 10 while any bottle exists). 259 pass.
