...
- 2026-07-15：**Shopify CSV 导出图片改用 Cloudinary URL**（用 subagent 驱动开发执行，每任务独立实现+审查）。根因：`export_shopify_inventory_csv` 的 `Product image URL`/`Variant image URL` 两列此前经 `request.build_absolute_uri` 输出本机局域网地址（如 `http://192.168.x.x:8000/media/...`），Shopify 的导入器从其自有服务器抓取图片 URL，无法访问私网地址，两列实际上一直是死链接。修复：新增 `stock/services/cloudinary_urls.py::product_image_cdn_url`（读取 `cloudinary_sync.py` 已写入的命名约定——每个产品一个 Cloudinary 资产、`public_id`=条码——纯字符串拼接，不发网络请求、不读凭据），`_build_shopify_image_url`（`stock/views.py`）改为优先返回 Cloudinary 交付 URL（`c_pad,b_white,w_1600,h_1600,q_auto/<条码>.jpg`，1:1 白底方图），未配置 Cloudinary 时回退到原本机绝对 URL。无图片的产品两列仍留空（不变）。无新增模型字段/迁移。新增 10 个测试（URL 构造 6 + CSV 导出 3 + 计 1），146/146 通过。文档同步 PRD F2.18.3（数据导出模块）、ARCHITECTURE stock/services 模块列表。
...
- 2026-07-13：**订单修正支持改店铺**（修复下错店铺的订单）。修正表单（仅管理员）新增 Store 下拉（列活跃店铺，默认选中订单当前店铺/活跃店铺）；`save_sale_order_correction` 改为「显式所选店铺优先」（编辑也生效），保存时把订单及其重建的全部明细行 `Sale.store` 一并改到所选店铺（`SaleOrderPayment` 无店铺字段不动、不涉及 AR）；`snapshot_sale_order` 快照加 `store_id/store_name`，审计日志记录 旧→新 店铺。库存全店共享不受影响；当日汇总由既有 Sale 信号按日期重算。新增测试（服务：选店铺移动订单+明细、不传保持原店铺、快照含店铺；视图：表单显示下拉、POST 改店铺移动订单），全测试通过。文档同步 PRD F2.9.5。
- 2026-07-13：**产品图自动镜像到 Cloudinary**（本地为主 + 镜像图床，供 Shopify 按条码拉图；用 subagent 驱动开发执行，每任务独立实现+审查）。`ProductImage` 新增/替换/删除时把该产品**主图**（`images.first()`）镜像到 Cloudinary（cloud `bulvpmzg`，**动态文件夹模式**：`public_id=<条码>`、`asset_folder=product_images/<品牌>`、`overwrite`，交付 URL `.../image/upload/<条码>.<ext>`；无图则删该资产）；本地存储改为按品牌分目录 `media/product_images/<品牌>/`（`upload_to` 回调 + 迁移 0032）。产出：①`stock/services/cloudinary_client.py`（薄封装 SDK：`is_configured/upload_image/delete_image`，`cloudinary` 懒加载）；②`stock/services/cloudinary_sync.py::sync_product_primary_image`（算主图→上传或删除，结果码 uploaded/deleted/no_barcode/error，异常不抛）；③`ProductImage` post_save/post_delete 信号（`CLOUDINARY_AUTO_SYNC` 门控、`on_commit`、异常只记日志不阻断保存；与现有 Shopify 信号并存）；④`manage.py sync_cloudinary_images`（**默认 dry-run**，`--apply/--brand/--barcode/--limit`）；⑤`settings.py` 加 `CLOUDINARY_CLOUD_NAME/API_KEY/API_SECRET/FOLDER/AUTO_SYNC`（环境变量），`requirements.txt` 加 `cloudinary==1.45.0`，新增 `docs/CLOUDINARY_SYNC.md`。全 mock 测试（客户端 4 + 同步 5 + 信号 3 + 命令 1 + 路径 3），119/119 通过、check 通过。**局限**：仅镜像主图、只管以后（不迁移旧图/旧资产）、不自动推 Shopify。**相关回填**：此前已手动把 112 个 Lattafa 商品图经 Cloudinary 挂到 Scentory 并全部上架（图/SEO/alt/ACTIVE/Online Store 就绪）。
