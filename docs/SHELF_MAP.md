# 货架状态图（Shelf map）

## 为什么不是「库存数量」

手机壳不数个数。店员站在货架前要在一秒内回答两件事：

1. **有没有货？**
2. **货架上方的盒子里还有没有？**

不是「还剩几个」。数个数是没人会长期维护的工作，而**没人维护的数字比没有数字更糟**——
它会让人相信一个错的数。

所以每个 **款式 × 型号 × 颜色** 只有一个状态：

| 状态 | 含义 | 图标 |
|---|---|---|
| `OUT` | 没货了 | `–`（虚线框） |
| `DISPLAY` | 货架上有，且**就这些** | `●`（绿） |
| `EXTRA` | 货架上有，**上方盒子里还有** | `▲`（蓝） |

卖完了点成 OUT；上方的货全部补到货架后，把 EXTRA 点成 DISPLAY。都不需要填数字。
点一下格子按 `OUT → DISPLAY → EXTRA → OUT` 循环——这正是现实中发生的顺序。

## 两个轴都是数据

网格的列**不是写死的颜色**。手机壳按颜色摆，钢化膜不是——膜按胶型和边型摆。
所以每个款式指定它的**轴**（`ShelfAxis`），列就是这个轴下的选项（`ShelfOption`）：

| 款式 | 轴 | 列 |
|---|---|---|
| Normal silicone | Colour | Black / Transparent / Blue / … |
| Tempered glass | Glue & edge | Full glue flat / Full glue curved / … |

加一种新货 = admin 或页面上加一个款式 + 选一个轴，**不改代码**。

**手机型号是全 app 共用的**（`DeviceModel`）。录一次 iPhone 17 Pro，
硅胶壳、MagSafe 壳、钢化膜的网格里**同时都有**——这正是重点。

## 页面结构：分类 → 类型 → 网格

```
手机壳          Normal silicone €1.20 | Clear TPU | MagSafe €3.50 | 翻盖
钢化膜          Tempered glass €0.80 | Hydrogel film
                 ────────────────────────────────
                 型号 × 颜色（或胶型/边型）网格
```

类型标签上**直接显示批发价**，报价时不用再去别处查。
这里**没有零售价**：壳和膜还不是产品、不走收银台。价格在 admin 里改。

## 改库存要先点 Edit

网格默认**只读**。点 Edit 才能改，改完点 Save 一次性提交，Cancel 全部还原。

这样做是因为货架页会在手机上被随手拿着看——只读时一个误触不会改掉库存状态。
改动在浏览器里攒着，Save 时**一个请求、一个事务**写完；
带着未保存的改动离开页面会有提醒。

## 不走 Add product

壳和膜**从来不是产品**：没有条码、没有价格、不进产品列表。
`ShelfStyle.category` 把款式挂到目录分类上，所以在新增产品页选中 Cases 时，
页面**直接给出对应网格的入口**，而不是一张不该填的表单。

日常操作只有两件事：
1. 出了新机型 → 在货架页加一行型号（自动排到正确位置）
2. 某个颜色卖完 / 补货 → Edit → 点格子 → Save

鼠标、键盘这类**有条码**的商品仍然走正常的产品流程（产品列表里
照片、零售价、批发价都照常显示）——货架页只管壳和膜。

## 数据结构

**不存** `"IP15 Pro: B(up), T"` 这种文字。四件事各自独立：

| 表 | 作用 |
|---|---|
| `ShelfAxis` | 列的含义：Colour / Glue & edge / … |
| `ShelfOption` | 具体一列：Black、Full glue curved… |
| `ShelfStyle` | 款式，指定自己的轴 + 对应的目录分类 |
| `DeviceModel` | 手机型号（**全 app 共用**，带品牌和别名） |
| `ShelfStock` | 款式 × 型号 × 选项 + 一个 `state`（唯一约束） |

**行是懒创建的**：从没上过货的格子根本没有行，读起来和 OUT 一样。

**扩展性**：`state` 故意不是两个 boolean。以后加 `quantity`、进货、出货、采购、
销售时，现有每一行**仍然有意义**，不需要推翻重来。真正的通用型号关系
（`CompatibilityGroup`）已经存在，随时可以接上。

## 排序

先按**发售年份**，再按 `DeviceModel.sort_key`（数字**补零**后存下来）。

只按名字排会有两个问题：`iPhone 11` 会排在 `iPhone 9` 前面（补零解决），
而 `iPhone X` 根本没有数字、会掉到 iPhone 16 后面（年份解决——X 回到 8 和 11 之间）。

结果就是货架上的顺序：`6 → 6s → SE(2016) → 7 → 8 → X → XR → XS → 11 → … → 16e`。

**没填年份的型号排在最后**（不是最前），这样手工新加、还没填年份的型号是显眼的，
而不是悄悄跑到列表头部。页面上加型号时**默认填当年**。

初始机型：`python manage.py seed_phone_models --apply`（iPhone 全系 + 常用简称别名）。

## 通用型号

现阶段**只写备注、不同步状态**（店里的决定）。备注是给货架前的人看的提醒
（"和 14 Pro 通用"），两个型号的状态各自独立标记。备注**可被搜索**。

## 权限

- **改状态**：任何登录用户。标记缺货是站在货架前的人的工作，不该等经理。
- **加型号 / 加颜色 / 加款式**：经理。

## 页面

`/shelf/`（侧栏 Shelf）。搜索框**边打边筛**，没有 Apply 按钮；
筛空的品牌标题会自动隐藏。型号列**横向滚动时固定**在左边。
**没有照片**——一个黑色壳的照片和另一个黑色壳的照片没有区别。

初始化：`python manage.py seed_shelf --apply`（默认 dry-run），
建 Normal silicone 款式和 10 个常用颜色，之后全部在页面上增改。
