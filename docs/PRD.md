...
| 角色 | 判定条件 | 典型权限 |
|---|---|---|
| **Employee（员工）** | 已登录的普通用户 | 进货/出货录入、查看产品列表（不含敏感数据）、考勤打卡、客户基本信息查询 |
| **Manager（经理）** | `is_staff=True` 或属于 "Managers" 组，或 `is_superuser` | 员工权限 + 查看成本/利润等敏感数据、产品增删改、供应商管理、AR 管理、团队考勤汇总、库存调整 API |
