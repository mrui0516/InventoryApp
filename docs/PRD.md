...
- **F2.2.17 产品销售历史专页（`/sales-history/`，仅经理）**：从产品详情"View full sales history"或直接进入。顶部**按产品名/条码搜索**（`q`）→ 列出匹配产品（含累计销量）→ 选中（`?product=<id>`）后展示该产品**完整销售路径**：产品头(图/品牌/条码/在库/对账 chip)、日期区间+店铺过滤、KPI(销量/营业额/订单数/均单价/利润*/在库/首末销售日)、**按店铺**与**按月**两张汇总表、**销售明细表**(逐行:时间/**店铺**/订单#链接/客户/数量/单价/小计/支付/利润*;补录不影响库存的行标记 no-stock、半透明)，以及复用组件 `_stock_ledger.html` 的**完整库存流水**(见 F2.5.2.10)。产品详情页 Sales History 相应精简为**仅最近 10 天**(按 `business_at`),超出部分显示条数并链接到本专页。仅经理可见（员工访问 302 回 `sales_records`）。
