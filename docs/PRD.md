...
- **F2.2.17 产品销售历史专页（`/sales-history/`，仅经理）**：从产品详情"View full sales history"或直接进入。顶部**按产品名/条码搜索**（`q`）→ 列出匹配产品（含累计销量）→ 选中（`?product=<id>`）后展示该产品**完整销售路径**：产品头(图/品牌/条码/在库/对账 chip)、日期区间+店铺过滤、KPI(销量/营业额/订单数/均单价/利润*/在库/首末销售日)、**按店铺**与**按月**两张汇总表、KPI，以及**销售明细与库存流水融合的单一时间线**——复用组件 `_stock_ledger.html`(`ledger_detailed=True`):在原有 Stock Ledger(采购+/销售−/调整± + 运行余额 + 对账横幅)基础上,销售行内联展示**店铺/客户(可点)/单价/小计/支付/利润***,订单#亦为链接;补录不影响库存的行半透明。这条完整时间线是**全量**的(上方过滤仅缩小汇总/KPI,不裁剪流水,以保证对账有效)。产品详情页 Sales History 相应精简为**仅最近 10 天**(按 `business_at`),超出部分显示条数并链接到本专页。仅经理可见（员工访问 302 回 `sales_records`）。
...
- **F2.6.1** 客户档案字段：NIF（葡萄牙税号，9 位数字，唯一）、姓名、**客户类型**、邮箱、电话、**地址**、备注。
  - **F2.6.1.1 客户类型（`Customer.kind`）**：`retail`（零售，默认）/ `revenda`（批发转售）。两者是不同的生意：Revenda 按批发价、按量拿货，并且是**未来批发目录唯一会给登录账号的客户类型**。
    ...
    - **三个新增客户入口全部可选类型**：收银台（`outbound`）、客户页（经理）、客户页（员工）。收银台是绝大多数客户被创建的地方，那里问不到就等于填不上。
    - 新建客户默认 Retail。快速新增接口收到未知类型值时同样落 Retail。
  - **F2.6.1.2 地址（`Customer.address`）**：自由文本（多行），葡萄牙地址是整块写的（`Rua X, nº 12, 2º Esq / 2700-123 Amadora`），且 Revenda 的送货地址是要抄到单据上的，不用于检索。三个新增入口和编辑表单都能填，客户详情页与联系方式并列展示。空值存 `''` 而非 `NULL`。
