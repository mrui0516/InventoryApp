...
| 角色 | 判定条件 | 典型权限 |
|---|---|---|
| **Employee（员工）** | 已登录的普通用户（非 `is_staff`/非 "Managers" 组/非超管） | 出货（POS）、查看/搜索/新增产品（不含敏感成本数据）、当日销售查看+订单号查对账、客户搜索+新增+订单对账、登录/登出自动打卡 |
| **Manager（经理）** | `is_staff=True` 或属于 "Managers" 组，或 `is_superuser` | 员工权限 + 查看成本/利润等敏感数据、进货、产品编辑、供应商管理、AR 管理、完整销售/客户分析页、团队考勤汇总、库存调整 API |
...
- **Products 对员工开放查看与新增**：员工可搜索/浏览产品列表、查看产品详情、新增产品；但**进货（Inbound）与编辑产品（Edit product）仍仅经理**。产品详情/列表中的成本、"Suppliers & cost" 比价区块、批次成本、销售历史/指标继续对员工隐藏，仅展示零售价/批发价。无审批/待确认工作流。
- **Sales（销售，员工视图）**：单日订单列表（默认今天，可切换日期），展示时间/客户/件数/支付方式/金额，无图表、无年度总览、无采购数据，按登录用户所在店铺范围。新增**按订单号搜索**：输入订单号可直接跳转到该订单详情（`sale_order_detail`，与经理共用同一详情/小票页），订单行也可点击进入详情——用于员工核对具体订单而不暴露经营分析数据。经理登录仍看到完整的 `/sales-records/` 页面（含图表、年度趋势、采购对比）。
- **Customers（客户，员工视图）**：无查询词时不展示任何客户列表（避免整表暴露）；输入姓名/电话/邮箱/NIF 关键字后仅展示匹配客户的姓名/电话/邮箱（不含消费统计/分析）。保留"新增客户"（沿用既有 AJAX 新增接口，弹窗内完成，无需离开搜索页）。点击某客户进入的详情页仅展示**该客户的订单列表**（日期/订单号/件数/支付方式/金额，链到订单详情）用于对账，不含消费分析图表、AR 余额、时间线。经理仍看到完整的客户详情页（图表/AR/时间线等）。
